...
final class TunnelController: ObservableObject {
    ...
    @Published var helperDetail = "Allow once so later Connect All does not ask for a password."
    ...
    static func connectTimeline(helperReady: Bool) -> [FlowStep] {
        [
            FlowStep(
                id: "allow",
                title: "Helper",
                detail: helperReady ? "Passwordless helper is ready." : "Tap Allow once — one Mac password.",
            ...
            FlowStep(id: "tunnel", title: "Tunnel", detail: "Wait for a utun interface.", state: .pending),
    ...
    var canDisconnectAll: Bool {
        (isActive || kerioTunnelSeen || sessionPhase == .connected) && !isBusy && sessionPhase != .connecting
    }
    ...
    var appVersion: String {
        Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1.7.5"
    }

    var menuBarSubtitle: String {
        var parts: [String] = []
        parts.append(helperReady ? "Helper ready" : "Allow once needed")
        if kerioTunnelSeen {
            ...
            DispatchQueue.main.async {
                ...
                if ready {
                    ...
                    self.helperDetail = HelperService.filesPresent
                        ? "Helper files exist but sudo -n still needs a password. Tap Allow once again."
                        : "Allow once with your Mac password. After that, Connect All stays silent."
                    ...
                    guard self.shouldUpdateStatusText() else { return }
                    self.statusText = "Step 1 of 2 — Allow once, then Connect All"
                }
            }
        }
    }
    ...
    func installHelper() {
        ...
        pinStatus("Allow once — enter your Mac password in the system dialog")
        ...
        guard fileManager.isReadableFile(atPath: installScript.path) else {
            ...
            updateStep("allow", .failed, "Installer missing — reinstall from KerioSplit-Mehrad.dmg")
        ...
        Task { @MainActor [weak self] in
            ...
            self.setBusy(false)
            self.appendLog(result.text.isEmpty ? (result.ok ? "Helper installed" : "Allow once failed") : result.text)
            self.helperReady = ready
            if ready {
                self.updateStep("allow", .done, "Passwordless helper verified for \(userName).")
                self.helperDetail = "Route helper is active. Connect All will not ask for a password."
                self.lastLoggedDiagnosis = nil
                self.pinStatus("Allow once complete — Connect All will not ask for a password")
                self.notify(title: "Setup complete", body: "Connect All can now apply split routes without a password.")
            } else {
                self.updateStep("allow", .failed, "Allow once did not verify. See Activity.")
                self.helperDetail = result.text.isEmpty
                    ? "Allow once did not finish."
                    : String(result.text.prefix(180))
                self.pinStatus("Allow once failed — see Activity", seconds: 12)
            }
        }
    }
    ...
    func uninstallHelper() {
        ...
        Task { @MainActor [weak self] in
            ...
            self.lastLoggedDiagnosis = nil
            self.updateStep("allow", .pending, "Helper removed. Allow once to install it again.")
            self.helperDetail = "Allow once with your Mac password. After that, Connect All stays silent."
    ...
    func connectAll() {
        ...
        guard helperReady else {
            pinStatus("Allow once first — that is the only password prompt", seconds: 10)
            updateStep("allow", .failed, "Connect All is locked until Allow once succeeds.")
            return
        }
        ...
        setBusy(true, watchdogSeconds: kerioConnectWaitSeconds + 30)
        ...
        updateStep("tunnel", .pending, "Waiting for utun after the VPN connects.")
        ...
        work = DispatchWorkItem { [weak self] in
            ...
            if !network.hasKerio {
                ...
                while Date() < deadline {
                    ...
                    DispatchQueue.main.async {
                        self.updateStep("tunnel", .running, "Still waiting for utun (\(remaining)s)")
    ...
    private func runEngine(
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            if useHelper {
                ...
                Tap Allow once first — that is the only password prompt.
    ...
    private func loadConnectedTimeline() {
        processSteps = [
            ...
            FlowStep(id: "tunnel", title: "Tunnel", detail: tunnelInterfaces.isEmpty ? "utun detected" : tunnelInterfaces.joined(separator: ", "), state: kerioTunnelSeen ? .done : .running),
    ...
    func syncTimelineFromReality() {
        ...
        if isActive {
            ...
            if kerioTunnelSeen {
                ...
                updateStep("tunnel", .done, tunnelInterfaces.isEmpty ? "utun detected" : tunnelInterfaces.joined(separator: ", "))
            }
        }
    }
    ...
    private nonisolated static func bringUpVPN(
        ...
        case .systemVPN:
            return StandardVPN.connectSystemVPN(named: systemVPNName)
        case .openvpnProfile:
            return StandardVPN.openOpenVPNProfile(openvpnPath, status: StandardVPN.probe())
        }
    }
    ...
    private nonisolated static func waitStatus(_ backend: ConnectBackend, remaining: Int, persistent _: Bool) -> String {
        ...
        case .systemVPN:
            return "Waiting for macOS VPN utun (\(remaining)s)"
        case .openvpnProfile:
            return "Waiting for OpenVPN utun (\(remaining)s)"
        }
    }
    ...
    private nonisolated static func tunnelDetail(_ network: (hasKerio: Bool, utuns: [String], hasHijack: Bool)) -> String {
        let ifaces = network.utuns.isEmpty ? "utun" : network.utuns.joined(separator: ", ")
        if network.hasHijack {
            return "\(ifaces) detected · full-tunnel hijack present"
        }
        return "\(ifaces) detected"
    }
