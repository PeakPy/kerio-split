...
struct SurfaceCard<Content: View>: View {
    ...
    var body: some View {
        content()
            ...
            .shadow(color: Brand.deep.opacity(0.06), radius: 12, y: 4)
            .shadow(color: Brand.primary.opacity(0.04), radius: 18, y: 6)
    }
}
        ...
        Button(action: action) {
            ...
            .background {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(destructive ? AnyShapeStyle(Brand.danger) : AnyShapeStyle(Brand.accentGradient))
            }
        }
        .buttonStyle(.plain)
        .disabled(busy)
...
struct DualActionBar: View {
    ...
    private func stroke(_ kind: Kind) -> Color {
        ...
        case (.card, .connect): return Brand.primary.opacity(0.35)
