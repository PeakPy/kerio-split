...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .alert("Disconnect All?", isPresented: $controller.showDisconnectConfirm) {
            ...
            Text("Turns off split routing and disconnects the VPN.")
        }
        .onAppear {
    ...
    private var sidebar: some View {
        ...
        .safeAreaInset(edge: .bottom, spacing: 0) {
            VStack(alignment: .leading, spacing: 6) {
                StatusPill(
                    text: controller.helperReady ? "Route helper ready" : "Install route helper",
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        SectionLabel(
                            ...
                            subtitle: controller.helperReady
                                ? "Passwordless routing is active for Connect All / Disconnect All."
                                : "Install once with your Mac password. After that, Connect All stays silent."
                        )
                        ...
                        HStack(spacing: 10) {
                            ...
                            Text(controller.helperReady ? "Route helper active" : "Route helper not installed")
                ...
                SurfaceCard {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text("Kerio Split \(controller.appVersion)")
                            .font(.system(size: 13, weight: .semibold, design: .rounded))
                            .foregroundStyle(Brand.ink)
                        Text("Companion for Kerio Control VPN — applies split routes after a real tunnel is up. Not a Kerio protocol client.")
                            .font(.system(size: 12))
                            .foregroundStyle(Brand.muted)
                            .fixedSize(horizontal: false, vertical: true)
